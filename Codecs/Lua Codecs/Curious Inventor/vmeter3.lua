...
--[[
		for q=1,6 do
			...
			remote.trace("\n")
				 ...
				 --]]	
    local retEvents={
		remote.make_midi("b0 ad yy zz",{ y = bytes[1], z = bytes[2], port=1 }),
		remote.make_midi("b0 ae yy zz",{ y = bytes[3], z = bytes[4], port=1 }),
		remote.make_midi("b0 af yy zz",{ y = bytes[5], z = bytes[6], port=1 }),
	}
	return retEvents
--		remote.make_midi("f0 66 66 66 14 0a 02 f7", { } ),
