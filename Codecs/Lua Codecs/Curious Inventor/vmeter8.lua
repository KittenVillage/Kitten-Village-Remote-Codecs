...
function remote_process_midi(event)
	...
	if ret~=nil then
		...
		if g_last_state_delivered~=ret.x then
--[[
			g_ledarray=DrawCursor(ret.x)
			local ledoutput = {}

			ledoutput ={SetLEDArray(g_ledarray)}
			-- must pass these to remote_deliver_midi() or it crashes
			processed_events={
...
function remote_set_state(changed_items)
	...
	for i,item_index in ipairs(changed_items) do
		...
		if item_index==g_led_index then
			...
			if g_last_state_delivered~=remote.get_item_value(item_index) then
				g_last_state=remote.get_item_value(item_index)
			
--[[			
				g_ledarray=DrawCursor(remote.get_item_value(item_index))
				local ledoutput = {}
...
--]]
			...
			--remote.trace(remote.get_item_value(item_index))
...
--[[
	...
	if g_delivered_led_state~=new_state then
		...
		--ret_events=Concat(state_events,processed_events)
		--table.insert(ret_events,state_events)
		--table.insert(ret_events,processed_events)
...
--]]		
		...
		state_event1=remote.make_midi("ad xx yy",{ x = ledoutput[1], y = ledoutput[2], port=1 })	
		state_event2=remote.make_midi("ae xx yy",{ x = ledoutput[3], y = ledoutput[4], port=1 })
		state_event3=remote.make_midi("af xx yy",{ x = ledoutput[5], y = ledoutput[6], port=1 })	

		table.insert(ret_events,state_event1)
		table.insert(ret_events,state_event2)
		table.insert(ret_events,state_event3)
